...
import {VERSION,SiteFooter} from './components/site';
...
export default function Home() {
  return <>
    ...
    <div className="topbar"><span>WORKWELL NEXT.JS DEMO · UK-wide occupational health delivery</span><span>01253 366156 · Blackpool</span></div>
    <header><div className="nav">
      <Link className="brand" href="/"><span className="mark">W</span><span><b>WorkWell {VERSION} by OHI</b><small>Demo environment · not production</small></span></Link>
      ...
      <div className="actions"><Link className="btn soft" href="/login">Demo portal</Link><Link className="btn primary" href="/what-do-i-need">Tell us what you need</Link></div>
    </div></header>
    <main id="main">
      <section className="hero"><div className="wrap hero-grid"><div><span className="kicker">DEMO · Occupational health, rebuilt around work</span><h1>Healthier people.<br/>Stronger workplaces.</h1><p className="lead">WorkWell by OHI makes occupational health easier to understand, buy, book and manage, from one employee referral to nationwide mobile health surveillance. This Next.js build is deliberately back in demo mode while workflows and service specifications are being refined.</p><div className="hero-actions"><Link className="btn primary" href="/services">Explore full service details</Link><Link className="btn dark" href="/login">Open demo portal</Link></div><div className="ticks"><span>✓ UK-wide mobile delivery</span><span>✓ Employer + employee journeys</span><span>✓ Demo clinician workspace</span></div></div>
      ...
      <section className="section"><div className="wrap"><span className="kicker">Full service specifications</span><h2 className="mt14">See what each occupational-health service actually involves.</h2><p className="lead">Each service page now sets out who it is for, preparation, clinical assessment, tests, delivery options, outputs, follow-up and limitations.</p><div className="grid3 services-grid">{services.map(([icon,name,desc,price,href])=><article className="card visual-card" key={name}><div className="art">{icon}</div><div className="card-body"><span className="badge">Demo service</span><h3 className="mt12">{name}</h3><p>{desc}</p><div className="price">{price}</div><div className="muted">Representative demo price</div><Link className="btn dark mt14btn" href={href}>View full service detail</Link></div></article>)}</div></div></section>
      <section className="section band"><div className="wrap grid2"><div><span className="kicker">Working demo portal</span><h2 className="mt14">Employer, employee and clinician workspaces.</h2><p>The demo now includes visible demo credentials, fictional client records, reports, examination status, test results and a clinician test-capture screen.</p><Link className="btn primary" href="/login">Open demo portal login</Link></div><div className="grid2"><div className="card"><h3>Employer</h3><p>Clients, people, reports, programme activity and services.</p></div><div className="card"><h3>Employee</h3><p>Appointments, preparation, documents and personal outcomes.</p></div><div className="card"><h3>Clinician</h3><p>Clinical queue, tests, notes, outcomes and report workflow.</p></div><div className="card"><h3>Demo safety</h3><p>All displayed people, results and reports are fictional demo data.</p></div></div></div></section>
    </main>
    <SiteFooter/>
  </>;
}
